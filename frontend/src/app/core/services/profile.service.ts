...
export class ProfileService {
  ...
  GetUserProfile(): Observable<UserProfileModel> {
    return this.http.get<UserProfileModel>(this.controllerUrl);
  }

  EditUserProfile(profile: UserProfileModel): Observable<UserProfileModel> {
    return this.http.put<UserProfileModel>(this.controllerUrl, profile);
  }

  ChangePassword(passwords: ChangePassword): Observable<any> {
    return this.http.put<any>(environment.apiURL + '/password/', passwords);
  }

  ChangeEmail(email: ChangeEmail): Observable<any> {
    return this.http.post<any>(environment.apiURL + '/email/', email);
  }

  DeleteUserProfile(): Observable<UserProfileModel> {
    return this.http.delete<UserProfileModel>(this.controllerUrl);
  }
}
